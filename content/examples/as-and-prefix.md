...
Writing IYP queries from scratch is daunting!.. But once you can read queries, you can easily modify them, and get the data you are looking for.
...
By clicking on any of the nodes you will see its properties in the right sidepanel (if you do not see the panel, click the `<` icon). It should look something like this:

{% include gallery-figure.html img="images/examples/2497-prefixes-properties.png" alt="" caption="" width="40%" %}

Here you can see that the nodes can have multiple types. This node has the generic `Prefix` type and the more specific `BGPPrefix` node type. To be more precise we can use the type `BGPPrefix` in our query.
...
1. `ORIGINATE` is not the only type of relationship between `AS` and `Prefix` nodes. For example, for RPKI we have the `ROUTE_ORIGIN_AUTHORIZATION` relationship between `AS` and `Prefix` nodes. Find prefixes that are announced by one AS and that have a ROA for another AS.
